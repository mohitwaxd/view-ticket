...
import type { DropdownOption, Enterprise, User, ZendeskTicket, ZendeskComment } from './types';
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  // Ticket preview properties
  ticketPreview: ZendeskTicket | null = null;
  ticketComments: ZendeskComment[] = [];
  isLoadingTicket: boolean = false;
  
  private queryParamsSubscription?: Subscription;
  private hasProcessedParams: boolean = false;
  ...
  ngOnInit(): void {
    // Only process params once on initial load
    if (!this.hasProcessedParams) {
      this.queryParamsSubscription = this.route.queryParams.subscribe(async (params) => {
        if (this.hasProcessedParams) return; // Prevent duplicate processing
  ...
  private async handleQueryParams(params: Params): Promise<void> {
    ...
    this.ticketCreatorName = params['myName'] || 'Support Team';
    this.ticketCreatorEmail = params['myEmail'] || 'support@telecrm.in';
    this.ticketId = params['ticket_id'] || null;
  ...
  async fetchTicketPreview(): Promise<void> {
    ...
    try {
      const [ticket, comments] = await Promise.all([
        this.telecrmService.getZendeskTicket(this.ticketId!),
        this.telecrmService.getZendeskTicketComments(this.ticketId!)
      ]);
  ...
  async onViewTicket(): Promise<void> {
    if (this.ticketId) {
      // Open specific ticket in agent interface
      this.returnToUrl = `${environment.zendeskUrl}/agent/tickets/${this.ticketId}`;
    } else if (this.selectedEnterprise?.id) {
      // Show filtered list of tickets by enterprise
      ...
      // Show all tickets
      this.returnToUrl = `${environment.zendeskUrl}/hc/en-us/requests?query=&page=1&selected_tab_name=my-requests`;
    }
